...
async function startProcessor(tabId, streamId, settings) {
  ...
  try {
    ...
    dialoguePresence.connect(preampGain);
    ...
    const processor = { tabId, context, stream, source, bassMacro, midMacro, trebleMacro, eqFilters, dialogueMudCut, dialoguePresence, preampGain, nightCompressor, masterGain, peakLimiter, settings: sanitizeSettings(DEFAULT_SETTINGS) };
  ...
  (async () => {
    switch (message.type) {
      ...
      case 'GET_PROCESSOR_DIAGNOSTICS': {
        ...
        sendResponse({ ok: true, active: true, contextState: processor.context.state, sampleRate: processor.context.sampleRate, nightReductionDb: processor.nightCompressor.reduction, limiterReductionDb: processor.peakLimiter.reduction, audioTracks: processor.stream.getAudioTracks().length }); return;
      }
