...
export class LiveAiKaraoke {
  ...
  async start() {
    ...
    const streamId = await chrome.tabCapture.getMediaStreamId({ targetTabId: this.targetTabId });
    ...
    if (this.context.sampleRate !== this.profile.sampleRate) {
      throw new Error(`Live AI Karaoke requires ${this.profile.sampleRate} Hz AudioContext; Chrome created ${this.context.sampleRate} Hz.`);
    }
  ...
  #acceptPcm(data) {
    ...
    if (!this.ring.push(left, right)) {
      this.#fallback('Live AI buffer reached its 20 s hard limit.');
      return;
    }
  ...
  async #pump() {
    ...
    try {
      ...
      const instrumental = await processLiveMdxWindow({
      ...
      if (this.firstChunk) {
        ...
        this.onStatus({
          state: decision.level === 'warning' ? 'warning' : 'live',
          message: `Live AI Karaoke started · RTF ${rtf.toFixed(2)}× · audio is ~${startupLatency.toFixed(1)} s behind the video.`
        });
      ...
      this.onMetrics({
        rtf,
        rollingRtf: this.rollingRtf,
        ringSeconds: this.ring.available / this.profile.sampleRate,
        queueSeconds: Math.max(0, this.nextPlaybackTime - this.context.currentTime),
    ...
    if (this.running && this.ring.available >= mdxChunkSize(this.profile)) {
      queueMicrotask(() => this.#pump().catch((error) => this.#fallback(error?.message ?? String(error))));
    }
  }
  ...
  #schedule(left, right) {
    ...
    const startAt = this.nextPlaybackTime > 0
      ? Math.max(this.nextPlaybackTime, this.context.currentTime + 0.03)
  ...
  async #fallback(reason) {
    ...
    this.onStatus({ state: 'fallback', message: `${reason} Switching to Fast Karaoke.` });
    await this.stop({ keepFallbackFlag: true });
    await this.onFallback(reason);
  }

  async stop({ keepFallbackFlag = false } = {}) {
    this.running = false;
    this.processing = false;
    this.ring.clear();
    this.nextPlaybackTime = 0;
    ...
    this.context = null;
    if (!keepFallbackFlag) this.fallingBack = false;
  }
}
