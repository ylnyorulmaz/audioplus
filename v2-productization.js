...
const aiLabLink = document.querySelector('.ai-lab-link');
const aiLabNote = document.querySelector('.ai-lab-teaser .control-note');
...
function formatFrequency(frequency) { return frequency >= 1000 ? `${frequency / 1000}k` : String(frequency); }
...
function renderSpectrum(snapshot) {
  ...
  spectrumMetrics.textContent = Number.isFinite(rms) && Number.isFinite(peak) ? `Input ${rms.toFixed(1)} dB RMS · Peak ${peak.toFixed(1)} dB` : 'Live input spectrum';
}

async function refreshState() {
  if (!activeTab) return;
  const response = await chrome.runtime.sendMessage({ type: 'GET_TAB_STATE', tabId: activeTab.id, siteKey: siteKeyFromUrl(activeTab.url) });
...
async function pollSpectrum() {
  ...
  if (!state?.enabled) { resetSpectrum('Enable Audio+ to view the live input spectrum.'); return; }
  polling = true;
  try {
...
function configureAiLabLauncher() {
  if (!aiLabLink) return;
  aiLabLink.removeAttribute('target');
  aiLabLink.textContent = 'Open Live AI Karaoke Lab';
  if (aiLabNote) aiLabNote.textContent = 'Real local UVR-MDX 2-stem separation with experimental live tab playback. Slow GPUs automatically drop to lightweight Fast Karaoke.';
  aiLabLink.addEventListener('click', async (event) => {
    ...
    await chrome.tabs.create({ url: chrome.runtime.getURL(`ai-lab.html${params.size ? `?${params}` : ''}`) });
  });
}

advancedPanel.addEventListener('toggle', () => {
  if (advancedPanel.open) refreshState().then(pollSpectrum).catch(console.error);
  else resetSpectrum(state?.enabled ? 'Open Advanced to view the live input spectrum.' : 'Enable Audio+ to view the live input spectrum.');
});
powerButton.addEventListener('click', () => { setTimeout(() => refreshState().then(pollSpectrum).catch(console.error), 300); });
document.addEventListener('visibilitychange', () => { if (document.visibilityState === 'visible') refreshState().then(pollSpectrum).catch(console.error); });
window.addEventListener('pagehide', () => { stopped = true; });

(async () => {
  buildBars();
  configureAiLabLauncher();
  const [tab] = await chrome.tabs.query({ active: true, currentWindow: true });
  if (!tab) return;
  activeTab = tab;
  await refreshState();
