...
test('settings clamp unsafe input including V2.1 dynamics', () => {
  const safe = sanitizeSettings({ bassDb:99, midDb:-99, preampDb:99, volume:999, dialogueBoost:999, nightMode:'nonsense', eqBands:Array(10).fill(99) });
  assert.equal(safe.bassDb,12); assert.equal(safe.midDb,-12); assert.equal(safe.preampDb,6); assert.equal(safe.volume,150);
  assert.equal(safe.dialogueBoost,100); assert.equal(safe.nightMode,'off'); assert.ok(safe.eqBands.every((value)=>value===12));
});
...
test('manifest is MV3 V2.1 release 0.5.0 with minimum permissions', async () => {
  const manifest=JSON.parse(await readFile(new URL('../manifest.json',import.meta.url),'utf8'));
  assert.equal(manifest.manifest_version,3); assert.equal(manifest.version,'0.5.0');
...
test('V2.1 graph includes dialogue filters and night compressor before master', async () => {
  const source=await readFile(new URL('../offscreen.js',import.meta.url),'utf8');
  ...
  assert.match(source,/light:.*threshold:\s*-24/s); assert.match(source,/strong:.*threshold:\s*-32/s);
});
