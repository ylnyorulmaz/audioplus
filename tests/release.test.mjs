...
test('peak protection and stable V2 processing graph remain intact', async () => {
  ...
  assert.match(source,/VOCAL_LOW_CROSSOVER_HZ = 200/); assert.match(source,/VOCAL_HIGH_CROSSOVER_HZ = 5500/); assert.match(source,/VOCAL_PRESENCE_CUT_DB = 6/); assert.match(source,/midPresence/); assert.match(source,/midAmount = aggressive/); assert.match(source,/case 'GET_SPECTRUM'/); assert.match(source,/case 'ANALYZE_AUDIO'/);
});
