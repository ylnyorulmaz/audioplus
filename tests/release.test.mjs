...
test('manifest is MV3 native side-panel 0.15.0 without broad tabs permission', async () => {
  const manifest=JSON.parse(await read('manifest.json'));
  assert.equal(manifest.manifest_version,3); assert.equal(manifest.version,'0.15.0');
...
test('toolbar action opens native side panel and remembers the selected source', async () => {
  const worker=await read('service-worker.js'); const panel=await read('sidepanel.html'); const target=await read('target-tab.js');
  assert.match(worker,/chrome\.action\.onClicked/); assert.match(worker,/chrome\.sidePanel\.open/); assert.match(worker,/audioPlus\.sidePanelTarget\./);
